...
namespace :cten do
	...
	desc "Generate the contact Excel file of Contacts to add to Denari Online"
  	task :generate_contact_excel => :environment do
  		...
  		row_count = 0
  		Contact.order("last_name ASC").where("address_1 <> '' AND (account_number = '' OR account_number IS NULL)").each do |c|
  			if c.country.iso3.downcase == "usa"
	  			...
	  			if c.id == 302
	  				worksheet.row(row_count).push "San Gabriel Community Church", "", "", "", "San Gabriel Community Church", "Church Family", cleaned(c.address_1), cleaned(c.address_2), cleaned(c.city), state, c.zip, country, c.email
	  			else
	  				worksheet.row(row_count).push "", cleaned(c.first_name), cleaned(c.last_name), cleaned(c.spouse_name), cleaned(c.family_name), cleaned(salutation), cleaned(c.address_1), cleaned(c.address_2), cleaned(c.city), state, c.zip, country, c.email
  				end
  			end
  		end
